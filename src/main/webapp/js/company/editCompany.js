...
layui.config({
    ...
    $("#companyName").blur(function () { //当元素失去焦点时发生 blur 事件 就是输入完用户名的时候 调用functino函数
        $.ajax({
            type: "get",
            dataType: "json",
            url: ctx + "/company/checkCompanyName/" + $("#companyName").val(),
            success: function (data) {
                if (data.code != 0) {
                    top.layer.msg(data.msg);//top.layer 表示在最顶层弹出窗口
                    $("#companyName").val(""); //清空原来的输入
                    $("#companyName").focus();//焦点重新定位到companyName
                }
            }
        });
    });

    form.on("submit(updCompany)", function (data) {
        //弹出loading
        var index = top.layer.msg('数据提交中，请稍候', {icon: 6, time: false, shade: 0.2});
        var msg;
        $.ajax({
            type: "post",
            url: ctx + "/company/updCompany",
