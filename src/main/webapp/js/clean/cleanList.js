    ...
    table.render({
        ...
        cols: [[ //表头
...

            {
                ...
                title: '打扫时间',
                align: 'center',
                templet: '<div>{{ formatTime(d.cleanTime,0,"yyyy-MM-dd hh:mm:ss")}}</div>'
            },
            {field: 'cleanLevel', title: '清洁情况', align: 'center', templet: '#levelTpl'},

            {field: 'flatsName', title: '公寓名称', align: 'center', templet: '<div>{{d.flats.flatsName}}</div>'},
            {field: 'name', title: '打扫人姓名', align: 'center', templet: '<div>{{d.person.name}}</div>'},
    ...
    table.on('tool(cleanList)', function (obj) {
        ...
        if (obj.event == 'del') {
            ...
            layer.confirm('真的删除吗？', function (index) {
                $.ajax({
                    url: ctx + "/clean/deleteClean/" + data.id,
                    type: "get",
                    dataType: "json",
                    success: function (d) {
                        if (d.code == 0) {
